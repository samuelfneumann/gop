...
// TestNormalRsampleTensor tests to ensure that consecutive runs of a
// graph on which Rsample() has been called on a Normal with a tensor
// mean and stddev result in different values being sampled.
func TestNormalRsampleTensor(t *testing.T) {
	...
	for i := 0; i < tests; i++ {
		dims := minSize + rand.Intn(maxSize-minSize)
...
// TestNormalRsampleScalar tests to ensure that consecutive runs of a
// graph on which Rsample() has been called on a Normal with a scalar
// mean and stddev result in different values being sampled.
func TestNormalRsampleScalar(t *testing.T) {
	...
	for i := 0; i < tests; i++ {
		...
		g := G.NewGraph()
		stddevNode := G.NewScalar(g, tensor.Float64,
			G.WithName(gop.Unique("stddev")))
		err := G.Let(stddevNode, stddev)
		if err != nil {
			t.Error(err)
		}

		meanNode := G.NewScalar(g, tensor.Float64,
		...
		// Ensure the correct batch size was sampled
		if sample1.Shape()[0] != batchSize {
			t.Errorf("incorrect batch size sampled: %v != %v",
				sample1.Shape()[0], batchSize)
		}
		...
		// Ensure the correct batch size was sampled
		if sample2.Shape()[0] != batchSize {
			t.Errorf("incorrect batch size sampled: %v != %v",
				sample2.Shape()[0], batchSize)
		}
